...
std::vector<int> with_substitutions::kMP(const std::string& str, const std::string& substr,
    ...
    std::vector<int> border =
        is_standart_or_advanced ? findPrefixFunction(substr) : findBorderFunction(substr);
    int k = 0;
    for (int i = 1; i <= n; ++i) {
